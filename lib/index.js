...
class SwaggerSmoke {
  ...
  async _testEndpointMethod(template, method, metadata) {
    ...
    if (this.options.only
      && !this.options.only.includes(`${method}:${template}`)) {
  ...
  async _testCase(method, template, metadata, testCase) {
    ...
    if (this.options.only && !this.options.only.includes(`${method}:${template}`)) {
    ...
    try {
      ...
      const body = await this._buildRequestBody(template, method, metadata.parameters, testCase.code);
      ...
      this._testFails++;
      console.log(chalk.red(`${testName}: Test init error: ${error}`));
  ...
  async _buildRequestBody(template, method, params, testCaseStatusCode) {
    if (!params)
      return;
      ...
      if (this.options.setParamsInBody) {
        ...
        for (const paramKey of paramsKeys) {
          fakeData = await this.setObjectParamValue(fakeData,
            paramKey, template, testCaseStatusCode, method);
        }
      }
      return fakeData;
    }
  ...
  async setObjectParamValue(object,
    ...
    if (paramMatches.length > 0) {
      const paramValue = await this.getParamValue(key, null, testCaseStatusCode, template, method);
