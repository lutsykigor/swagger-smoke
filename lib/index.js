...
class SwaggerSmoke {
    ...
    return this._api[method](this._basePath + url)
      ...
      .then((res) => {
        if (successResponse.metadata.schema
          && !this._schemaValidator.validate(res.body, successResponse.metadata.schema)) {
          //TODO: get all errors
  ...
  _getSuccessResponseMetadata(responses) {
    ...
    const metadata = responses[code];

    if (this.options.notRequiredPropertiesValidation) {
      this._setSchemaPropertiesAsRequired(metadata.schema);
    }
  ...
  _setSchemaPropertiesAsRequired(schema) {
      ...
      if (lodash.isArray(val)) {
        val.forEach((el) => {
          if (lodash.isObject(el)) {
            this._setSchemaPropertiesAsRequired(el);
          }
        });
      }
      if (lodash.isObject(key)) {
        this._setSchemaPropertiesAsRequired(schema[key]);
      }
    });
  }
